...
nocite: |
  @zobitzSupportingCodeData2025
  @zobitzNeonSoilFluxV2902025
---
...
Timeseries of soil surface flux ($F_{S}$) from field-measured (yellow lines) and modeled soil fluxes (green or purple lines) by the `neonSoilFlux` R package. Fluxes from the `neonSoilFlux` R package are separated by the diffusivity model used (Millington-Quirk or Marshall, @sec-compute-diffusivity). Individual vertical axis labels in the first column represent the measurement levels where the flux-gradient approach is applied (@sec-compute-soil-flux). Ribbons for modeled soil fluxes represent approximately $\pm$ 1 standard deviation. Results are reported in local time. WREF, SJER, and SRER were sampled in 2022, and UNDE, WOOD, and KONZ were sampled in 2024. Sites (columns) are arranged from left to right in terms of increasing mean annual temperature.
:::
...
In one of the six field sites, the *post hoc* $D_{a}$ estimate fell roughly between the two diffusion estimation methods. At UNDE, WOOD, WREF, and SJER, the median field estimate of diffusivity was lower than both of the other methods. At the driest site, SRER (@tbl-neon-sites), the median field estimate of diffusivity was higher than both of the other methods and values showed a large amount of variation (@fig-diffusivity-plot).
...
Our results suggest that the most important way to improve reliability of the flux estimate is to reduce the usage of gap-filled data. The current approach to gap filling in `neonSoilFlux` uses monthly mean data to gap fill---this approach decreases the ability of the estimate to be responsive to short-term pulses that occur with rapid weather shifts. All sites had more than 75% of half-hourly periods with no-gap filled measurements (Figure S1, Supplementary Information). At five out of six sites (all except SRER), we used at least some gap-filled measurements of Soil Water Content (SWC). At WREF, field data collection occurred following a severe rainstorm, with soils at the beginning of the sampling week near their water holding capacity, which can influence the soil moisture sensor accuracy. In general, we recommend that whenever possible, knowledge of local field conditions should influence analysis decisions in addition to any QA filtering protocols in the `neonSoilFlux` package.
